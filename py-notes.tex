...
\uterm{interactive mode} of the interpreter.
...
interpreter the \uterm{Python console}, or simply the \uterm{console}.
...
\uterm{expressions} and \uterm{statements}. Here we start with expressions. The
...
that it gives \uterm{precedence} to multiplication with respect to addition. If
...
will call these instances \uterm{objects}. We will have more to say about these
matters later, but let us fix the terminology from the beginning: Every
\uterm{object} is an instance of a \uterm{class}, which names the object's
\uterm{type}. If you are confused about objects, classes, and so on, at this
...
\item What \pyv{type} does is to first let the interpreter evaluate the
...
\item A handy function to be used with float objects is \pyv{round}:
...
\item Just like numbers, strings can be added by `\Verb|+|' and you can
...
function which is called a \uterm{method}. These functions operate relative to certain types of objects. For
...
\item We will encounter more string methods below.
...
\item A very simple example would be a function that takes a list and an object,
...
\item Try multiplying 12 with 6666666666 (twelve 6's in a row), first directly
on the Python interpreter, then by the program above.
...
\begin{itemize}
\item  Compute the Fibonacci numbers less than 100.
...
\item A very effective programming technique is \uterm{recursion}. A recursive
...
\begin{itemize}
\item[]
\item[a.] Write Python code that implements the iterative algorithm for
...
\item[b.] Write Python code implementing the recursive algorithm.
\begin{hide}
\begin{usolution}
\begin{ucodeframe}
...
\begin{itemize}
\item Programs are structured texts that provides instruction and/or information
to the computer. It is also useful to include some text in your program that is
meant for a programmer -- you, in the first place. The usual name for such
devices is \uterm{comment}. 
\item The simplest type of comment is:



\item Another possibility is the block comment structure:
